...
The Playwright-based discovery scanner lives in [`scanner/`](../scanner) and runs
**separately** from the frontend runtime — for example, as the scheduled GitHub
Action in `.github/workflows/scan.yml`. It drives a real, version-pinned
Chromium (see `docs/adr/0005-scanner-uses-real-chromium.md`), opening a fresh
browser context per consent state (`before-choice`, `reject-all`, each optional
category, `accept-all`) and recording cookies (incl. HttpOnly), storage,
IndexedDB, third-party requests, redirects, scripts, iframes, and pixels.
...
Generate `config.json` from the network with `wp kjeks scan-config` (it lists
every site with its URL, blog id, and current policy version):

```bash
wp kjeks scan-config --paths=/,/about > scanner/config.json
```

CI can skip the file entirely and fetch the site list over REST from
`GET /wp-json/kjeks/v1/scan-config` (auth: application password,
`manage_network`) with `node src/cli.js --config-url … --out ../scan`.

Output is one deterministic JSON file per site with a per-subsite diff (new /
changed / removed). See [`scanner/README.md`](../scanner/README.md) for the
config shape, Browser Run endpoint option, and limitations.
...
```bash
# CI / remote (application password via HTTP Basic; caller needs manage_network):
KJEKS_USER=admin KJEKS_APP_PASSWORD='xxxx xxxx xxxx xxxx' \
  node scanner/src/import.js --site https://network.example.com scan/*.json
...
cd scanner
npm ci                    # scanner dependencies
npx playwright install --with-deps chromium
BASE_URL=http://plugins.local/ npx playwright test   # no-tracking-before-consent
node src/cli.js --config config.json --out ../scan   # run a scan
```
